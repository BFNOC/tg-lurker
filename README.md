...
| 链接库 | 统一查看摘要和广告 Bio 中收集到的 URL，外链新标签页打开 |
| 摘要 | 按日期查看历史摘要 |
...
在 Web 管理面板的“设置 → LLM 上游与自动切换”中，可添加任意多个上游，并用“上移/下移”调整优先级。每个上游独立配置 Base URL、API Key、模型列表和 API 格式；模型列表每行一个，按填写顺序尝试。每张卡片都可从“测试模型”下拉框选择一个模型并点击“测试连接”；测试会使用当前填写但尚未保存的字段，且不会写入设置。
